...
**Per-disease attribution.** `diseaseSeverity` is built by grouping PHOTOS, not pixels: each photo already carries one diagnosis from the backend (`diseaseLabel`, gated once on that photo's own disease fraction), and its *whole* PDLA — every disease pixel it has — is attributed entirely to that one diagnosis. Photos are grouped by diagnosis and pooled (`Σ disease_pixels` of photos diagnosed `label`) over the plant's WHOLE leaf area (`Σ leaf_pixels` of every usable photo, not just that disease's own photos). Because every photo's whole PDLA lands in exactly one disease's numerator over that one shared denominator, `Σ diseaseSeverity` sums EXACTLY to the pooled `severity` (barring independent 2dp rounding of each term). There is no separate per-disease gate — each photo was already censored once, by the backend, before it can carry a non-Healthy diagnosis at all; a second, independent gate on pooled per-class pixel counts was the old model (see `aggregateSample.js`'s module docblock) and is gone. Even a Healthy-diagnosed photo still contributes its leaf area to every disease's shared denominator — it just contributes zero to any numerator.
...
- **`diseaseSeverity.leaf_blast + diseaseSeverity.brown_spot + diseaseSeverity.bacterial_blight` sums EXACTLY to `severity`** (barring independent 2dp rounding of each term) — see Plant-Level Pooling above. This is a property of per-photo dominant-disease attribution: every photo's whole PDLA is grouped under its one diagnosis over the same shared denominator, so the rows and the total are two views of the same partition, not independently gated quantities. The real limitation this leaves: a single photo showing two diseases at once is attributed ENTIRELY to its dominant diagnosis — a secondary disease visible in that one photo is invisible to `diseaseFindings` unless some *other* photo of the same plant was itself diagnosed with it. Multi-disease reporting is therefore only as fine-grained as the farmer's photo-taking (one diagnosis per photo), not as fine-grained as the pixels within a single photo.
...
- **The mask preview (`masks`, MaskInspectorModal) is a NEAREST subsample at 1/4 linear scale (1024px WORK space → 256px).** Isolated few-pixel lesions may be decimated away in the preview even though they are counted in the real measurement — **the preview is not evidence for the PDLA number**, which is computed from `class_pixels` at full WORK resolution before any preview is ever built. A block-max-pooling step that preserved tiny lesions in the preview was considered and rejected: it would insert a second, undocumented transform between the model and the viewer, and would systematically over-state disease relative to the reported PDLA.
- **Phase-2 mask-preview colors are a display palette invented for this viewer, encoding class identity only.** They carry no severity meaning and are unrelated to the green→red SES ramp in `sesScale.js` — a red region in the mask means "this class was predicted here," never "this point is severe." Don't conflate the two color systems when writing about either.
